	...
	while { _i < (count LV_ACS_activeGroups) } do {
		...
		if (_uns < 1) then {
			...
			switch (side _wGroup) do {
				case west: { LV_AI_westGroups = LV_AI_westGroups - [_wGroup]; };
				case east: { LV_AI_eastGroups = LV_AI_eastGroups - [_wGroup]; };
				case resistance: { LV_AI_indeGroups = LV_AI_indeGroups - [_wGroup]; };
			};
		} else {
