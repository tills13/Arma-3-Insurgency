...
if (!isDedicated) then {
	[] spawn {
		...
		if (INS_rev_displayRespawnLocationMarker == 1) then { [] spawn INS_rev_fnct_displayRespawnLocationMarker; };
	
		INS_rev_GVAR_body_before_dead = player; // Memorize player's body before dead
		...
		_playableUnits = playableUnits; // Initialize other players state (JIP, disabledAI, ...)
