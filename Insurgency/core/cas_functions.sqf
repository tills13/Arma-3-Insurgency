...
INS_CAS_spawnAircraft = {
	...
	_type = if (isNil (_this select 0)) then { _this select 0 } else { INS_CAS_airCraftTypes call BIS_fnc_selectRandom };
...
INS_CAS_notifyETA = {
	...
	(leader group driver _plane) sideChat format ["Cordinates recieved, CAS (%1) inbound - %2 seconds out", _casType, round (_dis / _speed)];	
};
...
INS_CAS_createOrdinance = {
	...
	_bomb = _type createVehicle [((getPos _plane) select 0), ((getPos _plane) select 1), ((getPos _plane) select 2) - 5];
	_bomb setDir ((_targetLoc select 0) - (getPos _bomb select 0)) atan2 ((_targetLoc select 1) - (getPos _bomb select 1));

	(leader group driver _plane) sideChat "JDAM away...";
	...
	_bomb setVelocity [_bVelX, _bVelY, (velocity _bomb select 2) - (85 + (abs ((_bomb distance _targetLoc) - 536) * 0.150))];
...
INS_CAS_finishCAS = {
	...
	player call INS_CAS_removeMenuItems;
